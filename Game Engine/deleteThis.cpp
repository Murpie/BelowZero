...
{
	{
		...
		for (int i = 0; i < 1; i++)
		{
		CharacterMovement movement = CharacterMovement(window);
		moveScript.push_back(movement);
		}
		*/

		//int numberOfCameras = 1; // Player?
		...
		{
		std::cout << std::endl;
		std::cout << gameScene.gameObjects.size() << std::endl << std::endl;
		...
		gameScene.gameObjects[gameScene.gameObjects.size() - 1].name = "Camera"; //+std::to_string(i);
		gameScene.gameObjects[gameScene.gameObjects.size() - 1].addComponent(&moveScript[i]);
		}
		*/
		...
		{
			gameScene.addEmptyGameObject();
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].name = "Light " + std::to_string(i);
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].addComponent(&lights[i]);
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].transform = glm::vec3(0, 0, 0); // from level file ?
		}
		...
		for (int i = 0; i < nrOfMeshes; i++)
		{
			...
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].name = meshName[i];
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].addComponent(&meshFilter[i]);
			gameScene.gameObjects[gameScene.gameObjects.size() - 1].addComponent(materialLibrary.getMaterial(i));
		}
